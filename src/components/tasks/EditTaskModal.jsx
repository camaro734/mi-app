...
export default function EditTaskModal({ task, onClose, onSubmit }) {
  ...
  const handleSubmit = (e) => {
    ...
    // Ajustar progreso automáticamente según el estado
    let adjustedProgress = formData.progress;
  ...
  const handleProgressChange = (value) => {
    ...
    // Ajustar estado automáticamente según el progreso
    if (numValue === 0) {
      setFormData(prev => ({ ...prev, status: 'Pendiente' }));
    } else if (numValue === 100) {
      setFormData(prev => ({ ...prev, status: 'Completado' }));
    } else if (numValue > 0 && prev.status === 'Pendiente') {
      setFormData(prev => ({ ...prev, status: 'En progreso' }));
    }
  };
            ...
            <CardContent>
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="bg-gray-50 p-4 rounded-lg">
                  <h4 className="font-medium text-sm text-gray-700 mb-2">Información de la tarea</h4>
                  <div className="grid grid-cols-2 gap-4 text-sm text-gray-600">
                    <div>
                      <span className="font-medium">ID:</span> {task.id}
                    </div>
                    <div>
                      <span className="font-medium">Creado:</span> {new Date(task.createdAt).toLocaleDateString('es-ES')}
                    </div>
                  </div>
                </div>
